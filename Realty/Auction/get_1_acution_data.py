...
def main():
    try:
        ...
        print("rstSiDoLists =>" , qrySelectSeoulTradeMaster ,strAddressSiguSequence )
        ...
        for rstSiDoList in rstSiDoLists:
            ...
            strAddressSiguSequence = str(rstSiDoList.get('sido_code'))
            ...
            while True:
                ...
                #
                cookies = AuctionMakeRequestHeader.makeCookies(strSidoCode)
                if cookies == False:
                    raise QuitException("__144cookies__")

                headers = AuctionMakeRequestHeader.makeHeader()
                if headers == False:
                    raise QuitException("__144headers__")

                json_data = AuctionMakeRequestHeader.makeJsonData(strSidoCode, pageNo)
                if json_data == False:
                ...
                print("dictGetDatas => ", type(dictGetDatas), dictGetDatas)
                ...
                if strIpCheckErrorKeyWord in dictGetDatas:
                    if dictGetDatas[strIpCheckErrorKeyWord] == False:
                        ...
                        logging.info(
                            SLog.Ins(Isp.getframeinfo, Isp.currentframe()) + "[strIpCheckErrorKeyWord: (" + str(strIpCheckErrorKeyWord) + ")")
                        continue
