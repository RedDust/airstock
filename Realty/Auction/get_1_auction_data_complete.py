...
def main():
    try:
        ...
        print("qrySelectSeoulTradeMaster => " , qrySelectSeoulTradeMaster)
        ...
        for rstSiDoList in rstSiDoLists:
            print("rstSiDoLists => " , rstSiDoLists)
            ...
            while True:
                ...
                #
                cookies = AuctionMakeRequestHeader.makeCompleteCookies(strSidoCode)
                if cookies == False:
                    raise QuitException("__144cookies__")

                headers = AuctionMakeRequestHeader.makeCompleteHeader()
                if headers == False:
                    raise QuitException("__144headers__")

                json_data = AuctionMakeRequestHeader.makeCompleteJsonData(strSidoCode, pageNo)
                ...
                response = requests.post(
                    'https://www.courtauction.go.kr/pgj/pgjsearch/selectDspslSchdRsltSrch.on',
                    cookies=cookies,
                    headers=headers,
                    json=json_data,
                )
                ...
                if strIpCheckErrorKeyWord in dictGetDatas:
                    if dictGetDatas[strIpCheckErrorKeyWord] == False:
                        logging.info(
                            SLog.Ins(Isp.getframeinfo, Isp.currentframe()) + "[strIpCheckErrorKeyWord: (" + str(
                                strIpCheckErrorKeyWord) + ")")
                        dictSwitchData = dict()
                ...
                for dictDltSrchResults in listDltSrchResults:
                    ...
                    cursorRealEstate.execute(sqlInsertCourtAuctionSpool,
                                             [strDocCD, strBoCd,strMaeGiil,strMaegyuljGiil,srnSaNo, strUniqueValue2Enc, strSidoName, strAuctionSeq,
                                              strSiguName, json_string])
