...
def main():
    ...
    try:
        ...
        strCourtAuctionUrl = "https://www.courtauction.go.kr/RetrieveRealAucRoadCodeList.ajax"

        # DB 연결
        ResRealEstateConnection = pyMysqlConnector.ResKtRealEstateConnection()
        cursorRealEstate = ResRealEstateConnection.cursor(pymysql.cursors.DictCursor)

        sqlCourtAuctionSelect = "SELECT * FROM " + ConstRealEstateTable_AUC.CourtAuctionAddressSiguTable
        ...
        for rstSiDoList in rstSiDoLists:
            ...
            CityName = str(rstSiDoList.get('sido_name'))
            strSiGuCode = str(rstSiDoList.get('sigu_code'))
            strSiGuName = str(rstSiDoList.get('sigu_name'))
            ...
            logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                           inspect.getframeinfo(inspect.currentframe()).lineno) +
                         "[CityName: (" + str(len(CityName)) + ")" + str(CityName))
            ...
            logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                           inspect.getframeinfo(inspect.currentframe()).lineno) +
                         "[strSiGuCode: (" + str(len(strSiGuName)) + ")" + str(strSiGuName))
            ...
            data = {
                'index': 'FB',
                'sidoCode': CityKey,
                'siguCode': strSiGuCode,
                'vowelCode': '00000_55203',
                'id4': 'idDongCode1',
            }
            ...
            for siguGroup in siguGroups:
                ...
                strRoadValue = str(siguGroup.get('value'))
                if len(strRoadValue) > 1:
                    strRoadName = str(siguGroup.get_text()).replace("\n","")
                    strUniqueCode = CityKey + "_" + strSiGuCode + "_" + strRoadValue + "_" + strRoadName
                    arrRoadCodes =  strRoadValue.split("|")
                    if len(arrRoadCodes) != 2:
                        raise Exception(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                          inspect.getframeinfo(inspect.currentframe()).lineno) +
                                        'len(arrRoadCodes) => ' + len(arrRoadCodes))  # 예외를 발생시킴

                    logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                   inspect.getframeinfo(inspect.currentframe()).lineno) +
                                 "[strUniqueCode: (" + str(len(strUniqueCode)) + ")" + str(strUniqueCode))
                    logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                   inspect.getframeinfo(inspect.currentframe()).lineno) +
                                 "[CityKey: (" + str(len(CityKey)) + ")" + str(CityKey))
                    logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                    ...
                    strRoadCode = str(arrRoadCodes[0])
                    strRoadType = str(arrRoadCodes[1])

                    logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                   inspect.getframeinfo(inspect.currentframe()).lineno) +
                                 "[strRoadCode: (" + str(len(strRoadCode)) + ")" + str(strRoadCode))
                    logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                   inspect.getframeinfo(inspect.currentframe()).lineno) +
                                 "[strRoadType: (" + str(len(strRoadType)) + ")" + str(strRoadType))
                    logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                   inspect.getframeinfo(inspect.currentframe()).lineno) +
                                 "[strRoadName: (" + str(len(strRoadName)) + ")" + str(strRoadName))

                    sqlCourtAuctionSelect  = " SELECT * FROM " + ConstRealEstateTable_AUC.CourtAuctionAddressRoadTable
                    sqlCourtAuctionSelect += " WHERE unique_code = %s"
                    cursorRealEstate.execute(sqlCourtAuctionSelect, (strUniqueCode))
                    logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                   inspect.getframeinfo(inspect.currentframe()).lineno) +
                                 "[sqlCourtAuctionSelect: (" + str(len(sqlCourtAuctionSelect)) + ")" + str(
                        sqlCourtAuctionSelect))
                    ...
                    if nResultCount < 1:
                        sqlCourtAuctionInsert = " INSERT INTO " + ConstRealEstateTable_AUC.CourtAuctionAddressRoadTable + " SET "
                        sqlCourtAuctionInsert += " unique_code= %s , "
                        ...
                        sqlCourtAuctionInsert += " road_code= %s , "
                        sqlCourtAuctionInsert += " road_type= %s , "
                        sqlCourtAuctionInsert += " road_name= %s , "
                        sqlCourtAuctionInsert += " modify_date= NOW() , "
                        sqlCourtAuctionInsert += " reg_date= NOW() "
                        logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                       inspect.getframeinfo(inspect.currentframe()).lineno) +
                                     "[sqlCourtAuctionInsert: (" + str(len(sqlCourtAuctionInsert)) + ")" + str(
                            sqlCourtAuctionInsert))
                        cursorRealEstate.execute(sqlCourtAuctionInsert, (strUniqueCode, CityKey, CityName, strSiGuCode, strSiGuName, strRoadCode, strRoadType, strRoadName))
                    else:
                        sqlCourtAuctionUpdate = " UPDATE " + ConstRealEstateTable_AUC.CourtAuctionAddressRoadTable + " SET "
                        sqlCourtAuctionUpdate += " modify_date= NOW()  "
                        sqlCourtAuctionUpdate += " WHERE  seq = %s "
                        logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                                       inspect.getframeinfo(inspect.currentframe()).lineno) +
                                     "[sqlCourtAuctionUpdate: (" + str(len(sqlCourtAuctionUpdate)) + ")" + str(sqlCourtAuctionUpdate))
                        nSequence = str(SwitchDataList.get('seq'))
                        cursorRealEstate.execute(sqlCourtAuctionUpdate, nSequence)
                    ...
                    dictSwitchData['data_3'] = strSiGuCode
                    dictSwitchData['data_4'] = strSiGuName
                    dictSwitchData['data_5'] = strRoadCode
                    dictSwitchData['data_6'] = strRoadName
                    LibNaverMobileMasterSwitchTable.SwitchResultUpdateV2(strProcessType, True, dictSwitchData)

            logging.info(GetLogDef.GerLine(inspect.getframeinfo(inspect.currentframe()).filename,
                                           inspect.getframeinfo(inspect.currentframe()).lineno) +
                         "END[CityKey: (" + str(len(CityKey)) + ")" + str(CityKey)+"][CityName: (" + str(len(CityName)) + ")" + str(CityName)+"---------------------------------")
        ...
        if strSiGuName is not None:
            dictSwitchData['data_4'] = strSiGuName
        if strRoadCode is not None:
            dictSwitchData['data_5'] = strRoadCode
        if strRoadName is not None:
            dictSwitchData['data_6'] = strRoadName
