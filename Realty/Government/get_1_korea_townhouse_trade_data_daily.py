        ...
        for rstSelectData in rstSelectDatas:
            ...
            for nLoop in range(intRangeStart, intRangeEnd):
                ...
                for objectBodyItem in objectBodyItemAll:
                    ...
                    if objectBodyItem.find('대지권면적') != None:
                        BLDG_AREA = str(objectBodyItem.find('대지권면적').text).strip()
                    if objectBodyItem.find('법정동') != None:
                        BJDONG_NM = str(objectBodyItem.find('법정동').text).strip()
                    if objectBodyItem.find('연립다세대') != None:
                        HOUSE_TYPE = str(objectBodyItem.find('연립다세대').text).strip().replace('\'',"")
                    ...
                    sqlSelectMOLIT = "SELECT * FROM "+ConstRealEstateTable_GOV.MolitTownHouseRealTradeMasterTable+" WHERE unique_key = %s "
                    ...
                    if intMolitCount < 1:
                        ...
                        #INSERT
                        sqlInsertMOLIT  = " INSERT INTO "+ConstRealEstateTable_GOV.MolitTownHouseRealTradeMasterTable+" SET "
                        ...
                        sqlSelectMOLITCancel = "SELECT * FROM "+ConstRealEstateTable_GOV.MolitTownHouseRealTradeCancelTable+" WHERE unique_key = %s "
                        cursorRealEstate.execute(sqlSelectMOLIT, (strUniqueKey))
                        intMolitCancelCount = cursorRealEstate.rowcount
                        if intMolitCancelCount < 0:
                            sqlInsertMOLITCancel  = " INSERT INTO "+ConstRealEstateTable_GOV.MolitTownHouseRealTradeCancelTable+" SET "
                        ...
                        sqlUpdateMOLIT = " UPDATE "+ConstRealEstateTable_GOV.MolitTownHouseRealTradeMasterTable+" SET "
