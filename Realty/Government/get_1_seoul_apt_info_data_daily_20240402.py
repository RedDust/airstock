...
try:
    ...
    while True:
        ...
        url = "http://openapi.seoul.go.kr:8088/" + init_conf.SeoulAuthorizationKey + "/json/OpenAptInfo/" + str(
            nStartNumber) + "/" + str(nEndNumber)
        print(GetLogDef.lineno(), "url>>>", type(url), url)

        # url을 불러오고 이것을 인코딩을 utf-8로 전환하여 결과를 받자.
        response = urllib.request.urlopen(url)
        json_str = response.read().decode("utf-8")

        print(GetLogDef.lineno(), "json_str>>>", type(json_str), json_str)
        ...
        if bMore is None:
            raise Exception(GetLogDef.lineno() + 'bMore => ' + str(bMore))  # 예외를 발생시킴
            break
        ...
        for list in jsonRowDatas:
            ...
            print("dictSeoulRealtyTradeDataMaster", type(dictSeoulRealtyTradeDataMaster),
                  dictSeoulRealtyTradeDataMaster)
            ...
            if nResultCount > 0:
                ...
                print(GetLogDef.lineno(), "strDOROJUSO >", "[" + str(nLoop) + "]", type(strDOROJUSO), strDOROJUSO)
                if len(strDOROJUSO) < 1:
                    strDBADRES_CITY = str(SwitchDataList.get('ADRES_CITY'))
                    strDBADRES_GU = str(SwitchDataList.get('ADRES_GU'))
                    strDBADRES_DONG = str(SwitchDataList.get('ADRES_DONG'))

                    strDOROJUSO = strDBADRES_CITY + " " + strDBADRES_GU + " " + strDBADRES_DONG

                    strDBADRES_RMNDR = str(SwitchDataList.get('ADRES_RMNDR'))
                    if len(strDBADRES_RMNDR) > 0:
                        strDOROJUSO += " " + strDBADRES_RMNDR

                    else:
                        strADRES_DORO_RMNDR = str(SwitchDataList.get('ADRES_DORO_RMNDR'))
                        if len(strADRES_DORO_RMNDR) > 0:
                            strDOROJUSO += " " + strADRES_DORO_RMNDR
                        else:
                            strAPT_NM = str(SwitchDataList.get('APT_NM'))
                            strDOROJUSO += " " + strAPT_NM

                if len(strDOROJUSO) < 1:
                    raise Exception(GetLogDef.lineno() + 'strDOROJUSO => ' + strDOROJUSO)  # 예외를 발생시킴
                    break

                resultsDict = GeoDataModule.getNaverGeoData(strDOROJUSO)

                print(GetLogDef.lineno(), "dictSeoulRealtyTradeDataMaster >", type(dictSeoulRealtyTradeDataMaster),
                      dictSeoulRealtyTradeDataMaster)

                print(GetLogDef.lineno(), "resultsDict >", type(isinstance(resultsDict, str)),
                      isinstance(resultsDict, str))
                print(GetLogDef.lineno(), "resultsDict >", type(resultsDict), resultsDict)

                if isinstance(resultsDict, dict) == False:
                    # if type(resultsDict) is dict:
                    print(GetLogDef.lineno(), "resultsDict >", type(resultsDict), resultsDict)
                    raise Exception(GetLogDef.lineno() + 'strResultCode => ' + strResultCode)  # 예외를 발생시킴
                    break

                strLongitude = str(resultsDict['x'])  # 127
                strLatitude = str(resultsDict['y'])  # 37

                # strLongitude = str(SwitchDataList.get('X_CODE'))#126.9596386
                # strLatitude = str(SwitchDataList.get('Y_CODE'))#37.5006676
                print(GetLogDef.lineno(), "strLongitude >", type(strLongitude), strLongitude)
                print(GetLogDef.lineno(), "strLatitude >", type(strLatitude), strLatitude)

                qryInfoUpdate = " UPDATE " + ConstRealEstateTable_GOV.SeoulAPTInfoTable + " SET "
                qryInfoUpdate += "  X_CODE = '" + strLongitude + "' "
                qryInfoUpdate += " , Y_CODE = '" + strLatitude + "' "
                qryInfoUpdate += " , geo_point = ST_GeomFromText('POINT(" + strLongitude + " " + strLatitude + ")', 4326, 'axis-order=long-lat') "
                ...
                for dictSeoulAptInfoDataMaster in dictSeoulRealtyTradeDataMaster.keys():
                    ...
                    if dictSeoulColumnInfoData[dictSeoulAptInfoDataMaster]['type'] == 'int' and \
                            dictSeoulRealtyTradeDataMaster[dictSeoulAptInfoDataMaster] == '':
                        dictSeoulRealtyTradeDataMaster[dictSeoulAptInfoDataMaster] = '0'

                    if dictSeoulColumnInfoData[dictSeoulAptInfoDataMaster]['type'] == 'datetime' and \
                            dictSeoulRealtyTradeDataMaster[dictSeoulAptInfoDataMaster] == '':
                    ...
                    qryInfoInsert += " " + dictSeoulAptInfoDataMaster + "='" + dictSeoulRealtyTradeDataMaster[
                        dictSeoulAptInfoDataMaster] + "', "

                strLongitude = str(dictSeoulRealtyTradeDataMaster['X_CODE'])
                strLatitude = str(dictSeoulRealtyTradeDataMaster['Y_CODE'])

                qryInfoInsert += "geo_point = ST_GeomFromText('POINT(" + strLongitude + " " + strLatitude + ")'), "
