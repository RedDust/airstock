...
def main():
    ...
    try:
        ...
        dtNowBaseTime = DateTime(2025,1,13,10,0,0)

        while True:

            intModifyMinute = int(dtNowBaseTime.minute) % 5

            if intModifyMinute!= 0:
                dtNowBaseTime = dtNowBaseTime - TimeDelta(minutes=intModifyMinute)


            dtNowStartTime = dtNowBaseTime - TimeDelta(minutes=5)
            dtNowEndTime = dtNowBaseTime


            if int(dtNowBaseTime.hour < 10):
                print("dtNowBaseTime.hour => ", dtNowBaseTime.hour)
                print("dtNowBaseTime.minute => ", dtNowBaseTime.minute)
                break

            # if int(dtNowBaseTime.hour) == 9 and int(dtNowBaseTime.minute) < 15:
            #     print("dtNowBaseTime.hour => ", dtNowBaseTime.hour)
            #     print("dtNowBaseTime.minute => ", dtNowBaseTime.minute)
            #     break

            if int(dtNowBaseTime.hour > 15):
                print("dtNowBaseTime.hour => ", dtNowBaseTime.hour)
                break


            if int(dtNowBaseTime.hour) == 15 and int(dtNowBaseTime.minute) >= 15:
                print("dtNowBaseTime.hour => ", dtNowBaseTime.hour)
                print("dtNowBaseTime.minute => ", dtNowBaseTime.minute)
                break


            listTimeDatas.append([dtNowStartTime, dtNowEndTime])


            dtNowBaseTime = dtNowBaseTime + TimeDelta(minutes=5)



        print("listTimeDatas => " , listTimeDatas)
        ...
        if intAffectedCount > 0:
            ...
            for rstSelectData in rstSelectDatas:
                ...
                for listTimeDataKey in listTimeDatas:
                    ...
                    print("listTimeDataKey => ", intTimeLoop, listTimeDataKey)
                    ...
                    intTimeLoop += 1
                    # slave_thread():
                    th1 = Thread(target=GetPrice.slave_thread, args=(logging, strStartHHMM, strEndHHMM, rstSelectData, strItemTableName))
                    th1.start()
                    th1.join()
