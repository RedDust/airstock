...
config_root = 'D:/PythonProjects/airstock/Stock/API/koreaInvestment/Config/'  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다.
...
def auth(svr='prod', product=_cfg['my_prod'], url=None):
    ...
    if saved_token is None:  # 기존 발급 토큰 확인이 안되면 발급처리
        ...
        print("json._getBaseHeader(p) >> ", _getBaseHeader())
