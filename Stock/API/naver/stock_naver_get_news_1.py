    ...
    try:
        ...
        if intAffectedCount > 0:
            ...
            for rstSelectData in rstSelectDatas:
                ...
                RealtyCallUrl = "https://search.naver.com/search.naver?where=news&ie=utf8&sm=nws_hty&query='"+strDBSectorsName+"'"
                RealtyCallUrl += "&sm=tab_opt&sort=1&photo=0&field=0&pd=0&ds=&de=&docid=&related=0&mynews=0&office_type=0&office_section_code=0&news_office_checked=&nso=so%3Add%2Cp%3Aall&is_sug_officeid=0&office_category=0&service_area=0"

                RealtyCallUrl = "https://search.naver.com/search.naver?sm=tab_hty.top&where=news&ssc=tab.news.all&query=%EB%91%90%EC%82%B0%EB%A1%9C%EB%B3%B4%ED%8B%B1%EC%8A%A4&oquery="
                RealtyCallUrl += "'"+strDBSectorsName+"'"
                RealtyCallUrl += "&tqi=i1q53lqVOswssL9%2Fbw4ssssssqV-080795&nso=so%3Add%2Cp%3A1d&mynews=0&office_category=0&office_section_code=0&office_type=0&pd=7&photo=0&service_area=0&sort=1"

                'https://search.naver.com/search.naver?sm=tab_hty.top&where=news&ssc=tab.news.all&query=%EB%91%90%EC%82%B0%EB%A1%9C&oquery=' \
                '&tqi=i1qQEsqVN8ossv0u69VssssstkN-139095&nso=so%3Add%2Cp%3Aall&mynews=0&office_category=0&office_section_code=0&office_type=0&pd=7&photo=0&service_area=0&sort=1'

                # RealtyCallUrl = parse.urlparse(RealtyCallUrl)
                ...
                rstMainTBobyElement = soup.select_one('#main_pack > section > div.api_subject_bx > div.group_news > ul')

                # '#sp_nws1 > div > div > div.news_contents > a.news_tit'

                rstMainTBobyLiElements = rstMainTBobyElement.select('li')

                for rstMainTBobyLiElement in rstMainTBobyLiElements:
                    print("-------------------------------------------------------")
                    strTitle = rstMainTBobyLiElement.select_one('a.news_tit')
